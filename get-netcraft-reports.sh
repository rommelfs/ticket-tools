...
LAST=`./rt list "id > $LASTPROCESSED and Queue='General' and (Status='new' or Status='open') and Subject like 'Issue' and Requestor.EmailAddress like '@netcraft.com'" -f ticket |grep -v "id" |tail -n 5000`
...
do
  ...
  then 
    ...
    URL=`./rt show $tn|grep -v 'Subject' | grep http -m 1 | cut -d " " -f 1` 
    #./rt show $tn 
    echo $URL
